function figures = plot_phase2_4_load_task_sweep(summaryTable, options)
%PLOT_PHASE2_4_LOAD_TASK_SWEEP Plot LoadTask response and scheduling data.
%
...
assert(~isempty(completed), ...
    "No completed Phase 2.4 cases are available to plot.");
...
figureHandle = figure( ...
    Name="Phase 2.4 LoadTask schedulability", ...
    Color="white");
layout = tiledlayout( ...
...
for rowIndex = 1:height(caseRows)
    ...
    if strlength(runFile) == 0 || ~isfile(runFile)
        warning("SDV:MissingPhase2_4Run", ...
    ...
    if ~isfield(savedData, "runArtifact") ...
            || ~isfield(savedData.runArtifact, "loadResponseTime_s")
    ...
    samples_ms = 1e3*double( ...
        savedData.runArtifact.loadResponseTime_s(:));
