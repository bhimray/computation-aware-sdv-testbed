...
communicationMetrics = summarizeTask( ...
    communicationTask, communicationTaskPeriod_s);

observedTotalCpuUtilization_pct = ...
    controlMetrics.ObservedCpuUtilization_pct ...
    + loadMetrics.ObservedCpuUtilization_pct ...
    + communicationMetrics.ObservedCpuUtilization_pct;
...
runArtifact.observedCpuUtilization = struct( ...
    Control_pct=controlMetrics.ObservedCpuUtilization_pct, ...
    Load_pct=loadMetrics.ObservedCpuUtilization_pct, ...
    Communication_pct=communicationMetrics.ObservedCpuUtilization_pct, ...
    Total_pct=observedTotalCpuUtilization_pct);
runArtifact.controlObservedCpuUtilization = ...
    controlMetrics.ObservedCpuUtilization;
runArtifact.loadObservedCpuUtilization = ...
    loadMetrics.ObservedCpuUtilization;
runArtifact.communicationObservedCpuUtilization = ...
    communicationMetrics.ObservedCpuUtilization;
...
executionTime_s = finiteColumn(task.Duration);
...
metrics = struct( ...
    ...
    ObservedCpuUtilization=executionTime_s/deadline_s, ...
    ObservedCpuUtilization_pct=100*finiteMean(executionTime_s/deadline_s), ...
...
function row = createSummaryRow( ...
    runArtifact, controlMetrics, loadMetrics, communicationMetrics)
...
record.ObservedTotalCpuUtilization_pct = ...
    runArtifact.observedCpuUtilization.Total_pct;
