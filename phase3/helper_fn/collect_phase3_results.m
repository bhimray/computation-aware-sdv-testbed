...
loadDeadline_s = metadataValue( ...
    metadata, "LoadTaskPeriod_s", 0.050);
...
communicationPeriod_s = metadataValue( ...
    metadata, "CommunicationTaskPeriod_s", 0.005);
...
supervisorPeriod_s = metadataValue( ...
    metadata, "SupervisorTaskPeriod_s", 0.010);
...
% Communication remains a configured constant. The supervisor can use a
% time-varying duration profile when the caller supplies one.
communicationTaskExecutionTime_s = metadataValue( ...
    metadata, "CommunicationTaskExecutionTime_s", 0.0005);
communicationTaskPeriod_s = communicationPeriod_s;

supervisorExecutionTime_s = metadataValue( ...
    metadata, "SupervisorTaskExecutionTime_s", 0.00005);

if isfield(metadata, "SupervisorTaskDurationSignal")
    supervisorDurationSignal = ...
        metadata.SupervisorTaskDurationSignal;

    supervisorPeriodSignal = struct( ...
        Time_s=supervisorDurationSignal.Time_s, ...
        Data=repmat( ...
            supervisorPeriod_s, ...
            numel(supervisorDurationSignal.Time_s), ...
            1));

    [supervisorUtilization, supervisorUtilizationTrace] = ...
        compute_phase3_control_utilization( ...
            supervisorDurationSignal, ...
            supervisorPeriodSignal, ...
            observationWindow_s);
else
    supervisorUtilization = ...
        supervisorExecutionTime_s/supervisorPeriod_s;
    supervisorUtilizationTrace = struct();
end

supervisorCpuUtilization_pct = 100*supervisorUtilization;
...
cpuUtilization.SupervisorTrace = supervisorUtilizationTrace;
...
runArtifact.phase = string( ...
    metadataValue(metadata, "Phase", "3"));
